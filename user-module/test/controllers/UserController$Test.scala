...
object UserController$Test extends PlaySpecification with Mockito {
  ...
  "Api" should {
    "should be reachable" in {
      ...
      val result = controllers.UserController(userDao).test(FakeRequest())
    ...
    "should add new user" in {
      ...
      val addUserAction = controllers.UserController(dao).addUser()
    ...
    "should get user" in {
      ...
      val result = controllers.UserController(dao).getUser("andrew@gmail.com").apply(FakeRequest())
